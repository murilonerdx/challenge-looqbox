...
public class HttpServicePokemon {
  ...
  public static List<SpotilightRequest> searchAllPokemon(Integer limit) {
    ...
    try {
      ...
      ResponseEntity<SpotilightRequest> pokemonRequest =
          template.exchange(URL + limit, HttpMethod.GET, entity, SpotilightRequest.class);

      searchAllSpotilightRequest =
          Collections.singletonList(Objects.requireNonNull(pokemonRequest.getBody()));
